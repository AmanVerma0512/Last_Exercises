...
class FormScore:
    ...
    def plot_y(self):
        if self.plot_flag:
            ...
            fig = plt.plot(range(len(self.y)), self.y)
            st.pyplot(fig)
            st.write("plot_y called")
            plt.show()
    ...
    def peaks(self):
        ...
        for i in range(start, end):
            ...
            #plt.show()
            # print("abcd")
            # plt.savefig(self.plot_dir+"peaks.png")
        return crests
    ...
    def peak_and_tempo(self):
        ...
        vals = range(len(y))
        if self.plot_flag:
            fig = plt.plot(xp, [y[i] for i in xp], ls="", marker="o", label="points")
            #st.pyplot(fig)
            #plt.savefig(self.plot_dir+"peaks&tempo.png")
            plt.show()
    ...
    def sudden_release(self):
        ...
        if self.plot_flag:
            ...
            # plt.savefig(self.plot_dir+"sudden-release.png")
            plt.show()
        return sud
    ...
    def jitter(self, window_size=4, delta=2, t0=2, x_dist_rel=0.2):
        ...
        if self.plot_flag:
            plt.plot(range(len(y)), y)
            plt.plot(range(len(y)), moving_averages)
        roots = jitter
        vals = range(len(y))
        mark = [vals.index(i) for i in roots]
        ...
        vals = range(len(y))
        if self.plot_flag:
            plt.plot(xp, [y[i] for i in xp], ls="", marker="x", label="points")
            # plt.savefig(self.plot_dir+"jitter.png")
        if self.plot_flag:
            plt.show()
        return jitter
    ...
    def smooth_blips(self, sm=2):
        ...
        vals = range(len(y))
        if self.plot_flag:
            plt.plot(prominent_peaks, [y[i] for i in prominent_peaks], ls="", marker="x", label="points")
            # plt.savefig(self.plot_dir+"smooth-blips.png")
            # plt.show()
